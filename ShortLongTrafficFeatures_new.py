...
class ShortTermLSTM(nn.Module):
    ...
    def forward(self, short_ttf):
        ...
        # V_short = []
        # y = 0
        cell_tensor = torch.zeros(size=[1, length, 12])#.to('cuda')
        for idx, cell in enumerate(short_ttf):
            ...
            # print(y)
            # """
            # h0_0 = torch.randn(1, 1, 100)  # (num_layers* 1,batch_size,hidden_size)
            # c0_0 = torch.randn(1, 1, 100)
            # h0_1 = torch.randn(1, 1, 100)
            # c0_1 = torch.randn(1, 1, 100)
            # h0_2 = torch.randn(1, 1, 100)
            # c0_2 = torch.randn(1, 1, 100)
            # outputs_0, (h_n, c_n) = self.lstm(inputs_0, (h0_0, c0_0))
            # outputs_1, (h_n, c_n) = self.lstm(inputs_1, (h0_1, c0_1))
            # outputs_2, (h_n, c_n) = self.lstm(inputs_2, (h0_2, c0_2))
            # """
...
class LongTermLSTM(nn.Module):
    ...
    def forward(self, long_ttf):
        ...
        length = len(long_ttf)
        cell_tensor = torch.zeros(size=[1, length, 4])#.to('cuda')
